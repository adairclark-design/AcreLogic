...
with open('src/components/MegaMenuBar.js', 'r') as f:
    content = f.read()

start_idx = content.find('export const MEGA_CATEGORIES')
end_idx = content.find('// ─── Component')

if start_idx == -1 or end_idx == -1:
    print("Could not find MEGA_CATEGORIES block")
    exit(1)

new_categories = """export const MEGA_CATEGORIES = [
    // ── All ───────────────────────────────────────────────────────────────────
    {
        label: 'All',
        emoji: '🌱',
    ...
    // ── Brassicas ─────────────────────────────────────────────────────────────
    {
        ...
        subcategories: [
            ...
            { label: 'Kale & Collards',      filter: c => c.id.includes('kale') || c.id.includes('collard') || c.id === 'kalettes' },
            ...
            { label: 'Other Brassicas',      filter: c => c.category === 'Brassica' && !c.id.includes('broccoli') && !c.id.includes('romanesco') && !c.id.includes('cabbage') && !c.id.includes('kale') && !c.id.includes('collard') && c.id !== 'kalettes' && !c.id.includes('cauliflower') && !c.id.includes('kohlrabi') && !c.id.includes('brussels') && !c.id.includes('choi') && !c.id.includes('choy') && !c.id.includes('tatsoi') && !c.id.includes('komatsuna') && !c.id.includes('mizuna') && !c.id.includes('asian_mix') },
        ],
    ...
    // ── Peppers ───────────────────────────────────────────────────────────────
    {
        ...
        filter: c => c.category === 'Nightshade' && c.id.includes('pepper'),
    },
    ...
    // ── Cucurbits ─────────────────────────────────────────────────────────────
    {
        ...
        subcategories: [
            ...
            { label: 'Pumpkins',             filter: c => c.id.includes('pumpkin') },
    ...
    // ── Legumes ───────────────────────────────────────────────────────────────
    {
        ...
        emoji: '🫘',
        subcategories: [
            { label: 'Bush Beans',           filter: c => c.category === 'Legume' && c.id.includes('bean') && (c.id.includes('bush') || c.id.includes('wax') || c.id.includes('french') || c.id.includes('green')) },
            ...
            { label: 'Peas',                 filter: c => c.category === 'Legume' && (c.id.includes('pea') || c.id === 'field_peas') && !c.id.includes('chickpea') && !c.id.includes('cowpea') },
            { label: 'Other Legumes',        filter: c => c.category === 'Legume' && !(c.id.includes('bean') && (c.id.includes('bush') || c.id.includes('wax') || c.id.includes('french') || c.id.includes('pole') || c.id.includes('runner') || c.id.includes('yard') || c.id.includes('green'))) && !(c.id.includes('pea') && !c.id.includes('chickpea') && !c.id.includes('cowpea')) },
        ],
    ...
    // ── Grains & Corn ─────────────────────────────────────────────────────────
    {
        ...
        subcategories: [
            ...
            { label: 'Grains & Amaranth',    filter: c => c.category === 'Grain' && !c.id.includes('corn') },
        ],
        filter: c => c.category === 'Grain',
    },
...
with open('src/components/MegaMenuBar.js', 'w') as f:
    f.write(new_content)

print("Updated MegaMenuBar.js successfully.")
