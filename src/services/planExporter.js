...
function cropCardHTML(item, imgSrc) {
    ...
    var cropIcon = imgSrc
        ? '<img src="' + imgSrc + '" style="width:32px;height:32px;border-radius:4px;object-fit:cover;flex-shrink:0;" alt="' + item.cropName + '">'
...
async function buildFamilyPlanHTML(planResult, familySize) {
    ...
    if (Platform.OS === 'web') {
        await Promise.all(
            planResult.supported.map(async function(item) {
                var src = CROP_IMAGES[item.id];
                // On web, resolved require() returns a URI string
                if (src && typeof src === 'string') {
                    var dataUrl = await imageToDataURL(src);
                    if (dataUrl) { imgMap[item.id] = dataUrl; }
                }
            })
        );
    }
    ...
    var cropCardsHTML = planResult.supported.map(function(item) {
        return cropCardHTML(item, imgMap[item.id] || null);
    }).join('');
    ...
    return '<!DOCTYPE html><html lang="en"><head>'
        ...
        + '<div class="footer">Generated by AcreLogic &middot; acrelogic-preview.pages.dev<br>'
...
function buildGardenPlanHTML(planResult, spaceResult, familySize) {
    ...
    return '<!DOCTYPE html><html lang="en"><head>'
        ...
        + '<div class="footer">Generated by AcreLogic &middot; acrelogic-preview.pages.dev</div>'
        + '</div></body></html>';
}
